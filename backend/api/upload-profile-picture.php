<?php
/**
 * AJAX Upload Endpoint for Admin Profile Pictures
 * 
 * This endpoint handles real-time admin profile picture uploads to Cloudinary via AJAX.
...
error_log("Admin Profile Picture AJAX Upload - User ID: " . $userId);
...
error_log("Admin Profile Picture AJAX Upload - Filename: " . $imageFilename);

// Folder path: Home/assets/public/admin-profile-images/
$folder = 'Home/assets/public/admin-profile-images';

try {
    // Upload to Cloudinary
    $result = uploadToCloudinary(
        $_FILES['image']['tmp_name'],
        $folder, // Folder path: Home/assets/public/admin-profile-images
    ...
    if ($result['success']) {
        ...
        if ($stmt->execute()) {
            ...
            // Log the activity
            logAdminActivity($conn, 'UPDATE', "Updated profile picture", 'users', $userId);
            ...
            error_log("Admin profile picture AJAX upload successful: " . $result['public_id']);
        } else {
        ...
        error_log("Admin profile picture AJAX upload failed: " . ($result['error'] ?? 'Unknown error'));
    }
} catch (Exception $e) {
    error_log('Admin profile picture AJAX upload exception: ' . $e->getMessage());
    http_response_code(500);
    echo json_encode([
        'success' => false,
