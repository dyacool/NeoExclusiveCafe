...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_bulk_order'])) {
    try {
        ...
        // Insert bulk order
        $insert_order = "INSERT INTO bulk_orders (name, contact, email, billing_address, order_type, delivery_address, purpose, date_needed, time_needed, note, total_amount) 
                        VALUES ('$name', '$contact', '$email', '$billing_address', '$order_type', '$delivery_address', '$purpose', '$date_needed', '$time_needed', '$note', '$total_amount')";
        ...
        if (mysqli_query($conn, $insert_order)) {
            ...
            $success_message = "Bulk order submitted successfully! Order ID: " . $bulk_order_id;
        } else {
...
// Get all products from database (all status)
$products_query = "SELECT id, name, price, status FROM products ORDER BY name";
        ...
        <form id="bulkOrderForm" method="POST" action="">
            ...
            <div class="form-section">
                ...
                <div class="form-group">
                    ...
                    <textarea id="purpose" name="purpose" rows="2" required placeholder="e.g., Corporate event, Wedding, Birthday party, etc."></textarea>
                </div>
                            ...
                            <div class="product-item">
                                <div class="product-checkbox">
                                    <input type="checkbox" 
                                           ...
                                           data-status="<?php echo isset($product['status']) ? $product['status'] : 'available'; ?>"
                                           class="product-select">
                                    <label for="product_<?php echo $product['id']; ?>">
                                        <strong><?php echo htmlspecialchars($product['name']); ?></strong>
                                        <span class="product-price">₱<?php echo number_format($product['price'], 2); ?></span>
                                        <?php if (isset($product['status']) && $product['status'] !== 'available'): ?>
