...
function showSaveButtons() {
  ...
  buttonContainer.style.cssText =
    "display: flex !important; gap: 10px; margin-top: 15px; justify-content: center; width: 100%;";
...
function handleCancelPreview(event) {
  ...
  const existingImage = document.querySelector(
    "#avatar img:not(#profile-image-preview)"
  );
  const existingPublicId =
    document.getElementById("remove-avatar-btn")?.dataset.publicId;
...
function initializeProfilePictureUpload() {
  ...
  if (avatarContainer && fileInput) {
    avatarContainer.addEventListener("click", (e) => {
      ...
      if (
        e.target.closest(".remove-avatar-btn") ||
        e.target.closest(".preview-buttons")
      ) {
        return;
      }
      fileInput.click();
