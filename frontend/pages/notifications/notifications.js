  ...
  fetch(`/frontend/pages/cart/get-order-details.php?order_id=${orderId}`)
    ...
    .then((data) => {
      if (data.success) {
        ...
        orderDetailsContent.innerHTML = `<div class="error-message">Failed to load order details: ${
            ...
            <table class="order-items-table">
                ...
                <tfoot>
                    <tr class="total-row">
                        ...
                        <td><strong>₱${(order.total_amount || subtotal).toFixed(
                          2
                        )}</strong></td>
                    </tr>
                </tfoot>
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function fetchNotificationDetails(notificationId) {
    return fetch(
      `/frontend/pages/notifications/fetch-notif.php?id=${notificationId}`
    )
      .then((response) => response.json())
      .then((data) => {
        if (data.status === "success") {
          ...
          throw new Error(
            data.message || "Failed to fetch notification details"
          );
        }
      });
  }
  ...
  function showNotificationModal(notification) {
    ...
    // Check if notification has order-related data or links - if not, just show simple modal
    const hasOrderDetails =
      notification.order_details &&
      Object.keys(notification.order_details).length > 0;
    const hasLink = notification.link || notification.order_id;

    if (
      (notificationType === "order" || notificationType === "order_update") &&
      hasOrderDetails
    ) {
      // Handle order notifications with details
      ...
      orderDetails.innerHTML = `
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <p><strong>Status:</strong> <span class="badge bg-primary">${
    ...
    } else if (notificationType === "promotion" && hasLink) {
      // Handle promotion notifications with links
      orderDetails.innerHTML = `
                <div class="row">
                    <div class="col-12">
                        ...
                        <p><strong>Notification ID:</strong> ${
                          notification.id || "N/A"
                        }</p>
                    </div>
                </div>
      ...
      // Handle simple notifications without order details or links (welcome messages, system alerts, etc.)
    ...
    if (modalElement) {
      try {
        if (!window.__notificationModalInstance) {
          window.__notificationModalInstance = new bootstrap.Modal(
            modalElement,
            {
              backdrop: true,
              keyboard: true,
      ...
      alert(
        "Unable to display notification details. Please refresh the page and try again."
      );
    }
  }
  ...
  if (notificationList) {
    ...
    function fetchAllNotifications(page = 1) {
      fetch(
        `/frontend/pages/notifications/fetch-notif.php?page=${page}&per_page=10`
      )
        .then((response) => response.json())
        .then((data) => {
          if (data.status === "success") {
            ...
            if (pagination) {
              ...
              prev.addEventListener("click", () =>
                fetchAllNotifications((data.page || 1) - 1)
              );
